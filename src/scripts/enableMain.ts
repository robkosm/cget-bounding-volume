...
PhysicsLoader("../../node_modules/ammojs3/builds", () => new Project(config));

const ms = new MainScene();

export function mount(container: Node) {
    if (container) {
        container.insertBefore(ms.renderer.domElement, container.firstChild);
    } else {
        ms.renderer.domElement.remove();
    }
}
